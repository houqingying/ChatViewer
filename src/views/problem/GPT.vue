...
//获取聊天记录
function getGPTChatMsg() {
  if (props.conversationInfo != null && props.conversationInfo.conversationId != null) {
    const param = props.conversationInfo.conversationId
    // 获取聊天记录
    getGptMessage(param).then((res) => {
      chatList.value = res;
    });
    // 如果属性改变时，传入了firstMsg，前端自动调用gpt接口
    if (props.conversationInfo.firstMessage) {
      inputMsg.value = props.conversationInfo.firstMessage
      sendText()
    }
  }
}
  ...
  .bottom {
      ...
      .chat-wrapper {
        ...
        .chat-friend {
          ...
          .chat-text {
            ...
            box-shadow:  2px 2px 10px var(--md-ref-palette-secondary90), 1px 1px 5px var(--md-sys-color-background-light);
        ...
        .chat-me {
          ...
          .chat-text {
            ...
            box-shadow:  2px 2px 10px var(--md-ref-palette-primary90), 1px 1px 5px var(--md-sys-color-background-light);
            font-size: 14px;
          }
          .info-time {
            margin: 10px 0;
    ...
    .chatInputs {
      ...
      .inputs {
        ...
        border-radius: 10px;
        border: 1.5px solid rgb(34, 135, 225);
        box-sizing: border-box;
        transition: 0.2s;
        font-size: 14px;
